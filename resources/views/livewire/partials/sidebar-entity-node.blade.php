...
<div wire:key="entity-{{ $node['entity_id'] }}"
     x-data="{ open: localStorage.getItem('correspondence.entity.' + {{ $node['entity_id'] }}) === 'true' }"
     class="flex flex-col">
    {{-- Entity-Zeile --}}
    <button type="button"
            @click="open = !open; localStorage.setItem('correspondence.entity.' + {{ $node['entity_id'] }}, open)"
        ...
        <span class="ml-auto text-[10px] tabular-nums text-[var(--ui-muted)] opacity-60">{{ $node['total_threads'] }}</span>
    </button>

    {{-- Aufgeklappter Inhalt --}}
    <div x-show="open" x-collapse class="flex flex-col ml-3 border-l border-[var(--ui-border)]">
        {{-- 1. Eigene Threads --}}
        @foreach($node['threads'] as $thread)
            <a wire:key="entity-{{ $node['entity_id'] }}-thread-{{ $thread->id }}"
        ...
        @foreach($node['children_by_type'] as $typeGroup)
            <div wire:key="entity-{{ $node['entity_id'] }}-type-{{ $typeGroup['type_id'] }}"
                 x-data="{ groupOpen: localStorage.getItem('correspondence.entity.' + {{ $node['entity_id'] }} + '.type.' + {{ $typeGroup['type_id'] }}) !== 'false' }"
                 class="flex flex-col">
                {{-- Typ-Label (nur wenn mehrere Gruppen oder eigene Threads) --}}
                @if($node['children_by_type']->count() > 1 || $node['threads']->isNotEmpty())
                    <button type="button"
                            @click="groupOpen = !groupOpen; localStorage.setItem('correspondence.entity.' + {{ $node['entity_id'] }} + '.type.' + {{ $typeGroup['type_id'] }}, groupOpen)"
